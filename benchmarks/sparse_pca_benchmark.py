...
class SparsePCABenchmark:
    ...
    def benchmark_method_comparison(self):
        ...
        for config in test_configs:
            ...
            print(
                f"    Estimated memory: sparse={memory_est['sparse_matrix']:.2f}GB, "
    ...
    def _benchmark_single_method(
        self, X_sparse, method_config, matrix_config, n_comps=50
    ):
        ...
        if method == "standard_dense":
            ...
            config = SparsePCAConfig(memory_limit_gb=16.0, use_randomized_svd=True)
        ...
        for factor in scaling_factors:
            ...
            try:
                ...
                result = {
                    ...
                    "efficiency_ratio": memory_used
                    / (memory_est["truncated_svd"] * 1024),
                }

                print(
                    f"    Actual: {memory_used:.1f}MB, "
    ...
    def benchmark_optimization_methods(self):
        ...
        print(
            f"  Test matrix: {X_sparse.shape[0]}x{X_sparse.shape[1]}, "
            f"nnz={X_sparse.nnz:,}"
        )

        optimization_configs = [
            {"name": "Default", "config": SparsePCAConfig()},
            {
                "name": "Randomized SVD",
                "config": SparsePCAConfig(
                    use_randomized_svd=True, n_oversamples=20, n_iter=4
                ),
            },
            {
                "name": "Memory Constrained",
                "config": SparsePCAConfig(memory_limit_gb=4.0, chunk_size=5000),
            },
            {
                "name": "High Performance",
                "config": SparsePCAConfig(
                    memory_limit_gb=32.0, use_randomized_svd=True, n_oversamples=15
                ),
            },
        ]
    ...
    def benchmark_centering_vs_no_centering(self):
        ...
        test_sizes = [(5000, 2000, "Medium"), (15000, 5000, "Large")]
    ...
    def save_results(self, filename="sparse_pca_benchmark_results.csv"):
        ...
        if self.results:
            ...
            if "time" in df.columns:
                ...
                print(
                    f"   Fastest method: {df.loc[df['time'].idxmin(), 'method']} "
                    f"({df['time'].min():.2f}s)"
                )
    ...
    def print_summary(self):
        ...
        method_results = [
            r for r in self.results if "method" in r and r.get("status") == "success"
        ]
        ...
        print(
            f"❌ Failed benchmarks: {len([r for r in self.results if r.get('status') == 'failed'])}"
        )
