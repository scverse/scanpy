...
- {func}`~scanpy.pp.normalize_total` replaces `sc.pp.normalize_per_cell`, is more efficient and provides a parameter to only normalize using a fraction of expressed genes {smaller}`S Rybakov`
