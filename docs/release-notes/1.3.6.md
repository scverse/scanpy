...
- the [UCSC Single Cell Browser] requires exporting via {func}`~scanpy.io.write_cellbrowser` (introduced here as `scanpy.external.exporting.write_cellbrowser`) {smaller}`M Haeussler`

#### Code design

- {func}`~scanpy.pp.highly_variable_genes` supersedes `sc.pp.filter_genes_dispersion`, it gives the same results but, by default, expects logarithmized data and doesn’t subset {smaller}`A Wolf`
