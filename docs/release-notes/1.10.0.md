...
* {func}`scanpy.tl.leiden` now relies by default on `igraph` fully.  `leidenalg`'s implementation is still available via `use_igraph` when set to `False`, although other defaults have now changed.  {pr}`2815` {smaller}`I Gold`
