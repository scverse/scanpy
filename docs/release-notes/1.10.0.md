...
* {func}`scanpy.tl.leiden` now relies by default on `igraph` fully.  `leidenalg`'s implementation is still available via `use_leidenalg` when set to `True`, although other defaults have now changed.  {pr}`2815` {smaller}`I Gold`
