...
* {func}`scanpy.tl.dendrogram` now accepts and axis or dim argument to enable dendrograms on obs or var {pr}`2770` {smaller}`D Burkhardt`
...
* Fix `sc.tl.dendrogram` ignoring passed var_names
