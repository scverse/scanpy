...
* {func}`scanpy.tl.leiden` now relies by default on `igraph` fully.  `leidenalg`'s implementation is still available via `backend` when set to `leidenalg`, although other defaults have now changed.  {pr}`2815` {smaller}`I Gold`
