...
def test_read_write_hdf5():
    ...
    write('./test.h5', adata)
    adata = read('./test.h5')
    assert pd.api.types.is_categorical(adata.obs['sanno1'])
    assert pd.api.types.is_string_dtype(adata.obs['sanno2'])
    assert adata.obs.index.tolist() == ['name1', 'name2', 'name3']
    assert adata.obs['sanno1'].cat.categories.tolist() == ['cat1', 'cat2']
