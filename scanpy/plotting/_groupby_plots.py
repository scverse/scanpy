...
class HeatMap(BasePlot):
    ...
    **kwds
    ...
    -------
    >>> import scanpy as sc
    >>> from scanpy.plotting._groupby_plots import HeatMap
    >>> adata = sc.datasets.pbmc68k_reduced()
    >>> markers = ['C1QA', 'PSAP', 'CD79A', 'CD79B', 'CST3', 'LYZ']
    >>> HeatMap(adata, markers, groupby='bulk_labels').add_dendrogram().swap_axes().show


    See also
    --------
    :func:`~scanpy.pl.heatmap`
    :func:`~scanpy.tl.rank_genes_groups_heatmap` to plot marker genes identified using the :func:`~scanpy.tl.rank_genes_groups` function.
    """
    ...
    def __init__(
        ...
        **kwds,
        ...
        if show_gene_labels is None:
            if len(var_names) <= 50:
                self.show_gene_labels = True
            else:
                self.show_gene_labels = False
                logg.warning(
        ...
        if groupby + "_colors" in adata.uns:
            self.groupby_colors = adata.uns[groupby + "_colors"]
        else:
            self.groupby_colors = None

        self.obs_tidy = self.obs_tidy.sort_index()
    ...
    def _mainplot(self, ax):
        ...
        fig, heatmap_gs = make_grid_spec(
            ax, nrows=1, ncols=2, width_ratios=[0.1, 1],
        )
        groupby_ax = fig.add_subplot(heatmap_gs[0])
        heatmap_ax = fig.add_subplot(heatmap_gs[1])

        im = heatmap_ax.imshow(self.obs_tidy.values, **kwds)
        heatmap_ax.set_ylim(self.obs_tidy.shape[0] - 0.5, -0.5)
        heatmap_ax.set_xlim(-0.5, self.obs_tidy.shape[1] - 0.5)
        heatmap_ax.tick_params(axis='y', left=False, labelleft=False)
        heatmap_ax.set_ylabel('')
        heatmap_ax.grid(False)

        if self.show_gene_labels:
            labels = _matrix.columns.tolist()
            heatmap_ax.tick_params(axis='x', labelsize='small')
            heatmap_ax.set_xticks(np.arange(len(labels)))
            heatmap_ax.set_xticklabels(labels, rotation=90)
        else:
        ...
        ticks, labels, groupby_cmap, norm = _plot_categories_as_colorblocks(
            groupby_ax, self.obs_tidy, colors=self.groupby_colors, orientation='left'
        )
        # TODO how to pass `ticks` to plot_dendrogram?

        # add lines to main heatmap
        line_positions = (
            np.cumsum(self.obs_tidy.index.value_counts(sort=False))[:-1] - 0.5
        )

        heatmap_ax.hlines(
            line_positions,
            -0.05, 1.05, transform=heatmap_ax.get_yaxis_transform(),
            lw=0.6,
            zorder=10,
            clip_on=False,
        )
