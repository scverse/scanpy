...
def spatial(
    ...
    *,
    ...
    img_key: Union[str, tuple, None, Empty] = _empty,
    crop_coord: Tuple[int, int, int, int] = None,
    alpha_img: float = 1.0,
    bw: bool = False,
    **kwargs,
    ...
    If `show==False` a :class:`~matplotlib.axes.Axes` or a list of it.
    """
    # check if any of the images slot is a list (if True, it means that it's a concatenated anndata)
    # if no batch_key is provided, return error
    if (
        any(
            [
                isinstance(adata.uns["images"][k], list)
                for k in adata.uns['images'].keys()
            ]
        )
        and batch_key is None
    ):
        raise ValueError(
            f"It seems that the AnnData object contains multiple tissues. \n"
            f"But`batch_key` is {batch_key}, `batch_key` in order to plot them. \n"
        )
    # if any of the images slot is a list and batch_key is provided,
    # return all available tissues
    elif (
        any(
            [
                isinstance(adata.uns["images"][k], list)
        ...
        and img_key is _empty
    ):
        logg.warning(
            f"It seems that the AnnData object contains multiple tissues. \n"
            f"Specify `'batch_key'` and `'img_key'` to plot a single tissue\n"
            f"All tissues will be plotted"
        )
        ...
        for idx in range(0, len(adata.uns['images'][img_key])):
            # if the anndata was generated by concatenating visium
            # and normal scRNA-seq, skip the scRNA-seq dataset
            if adata.uns["images"][img_key][idx] is None:
                pass
            else:
                new_adata = adata[adata.obs[batch_key] == str(idx), :]
                new_adata.uns["images"][img_key] = adata.uns["images"][img_key][idx]
                new_adata.uns["scalefactors"][f"tissue_{img_key}_scalef"] = adata.uns[
                    "scalefactors"
                ][f"tissue_{img_key}_scalef"][idx]
                new_adata.uns['scalefactors']['spot_diameter_fullres'] = adata.uns[
                    'scalefactors'
                ]['spot_diameter_fullres'][idx]
                embedding(
                    new_adata,
                    'spatial',
                    img_key=img_key,
        ...
        return
    # if any of the images slot is a list, batch_key is provided,
    # and img_key is a tuple, return the tissue
    elif (
        any(
            [
                isinstance(adata.uns["images"][k], list)
