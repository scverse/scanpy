...
def palantir( adata, **kargs ):
	...
	:Return objects will be pushed to adata:
		`data_df`			--> `adata.obsm['palantir_norm_data']`

		`pca_projections`	--> `adata.obsm['palantir_pca_projections']`

		`var_r`				--> `adata.uns['palantir_pca']['variance_ratio']`

		`dm_res`			--> `adata.uns['palantir_diff_maps']['T']`,
								`adata.uns['palantir_diff_maps']['EigenValues']`,
								`adata.obsm['palantir_dm_EigenVectors']`

		`ms_data`			--> `adata.obsm['palantir_ms_data']`

		`tsne`				--> `adata.obsm['palantir_tsne']`

		`imp_df`			--> `adata.obsm['palantir_imp_df']`
	...
	*Diffusion maps*
	...
	class _wrapper_cls( object ):
		...
		def __init__( self ,
			...
			adata.obsm['palantir_pca_projections'] = np.array(self.pca_projections)
			...
			adata.uns['palantir_diff_maps']['T'] = self.dm_res['T']
			adata.uns['palantir_diff_maps']['EigenValues'] = self.dm_res['EigenValues']
			adata.obsm['palantir_dm_EigenVectors'] = np.array(self.dm_res['EigenVectors'])
			adata.obsm['palantir_ms_data'] = np.array(self.ms_data)

			# tSNE visualization
			logg.info('tSNE in progress ...', r=True)
			self.tsne = self.palantir.utils.run_tsne(self.ms_data)
			adata.obsm['palantir_tsne'] = np.array(self.tsne)

			# MAGIC imputation
			logg.info('imputation in progress ...', r=True)
			self.imp_df = self.palantir.utils.run_magic_imputation(self.data_df, self.dm_res)
			adata.obsm['palantir_imp_df'] = np.array(self.imp_df)
