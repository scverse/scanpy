...
def paga_compare_paths(
    ...
    for (r, s) in itertools.combinations(leaf_nodes1, r=2):
        ...
        if len(path1) >= len(path2):
            ...
            path_compare = path2
            path_compare_id = 2
            path_compare_orig_names = [[orig_names2[int(s)] for s in link] for link in path_compare]
            path_mapped_orig_names = [[orig_names2[int(s)] for s in link] for link in path_mapped]
        else:
            path_mapped = [asso_groups2[link] for link in path2]
            path_compare = path1
            path_compare_id = 1
            path_compare_orig_names = [[orig_names1[int(s)] for s in link] for link in path_compare]
            path_mapped_orig_names = [[orig_names1[int(s)] for s in link] for link in path_mapped]
