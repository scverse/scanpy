...
def trimap(
    ...
    if 'neighbors' not in adata.uns:
        raise ValueError(
            'Did not find \'neighbors/connectivities\'. Run `sc.pp.neighbors`' 
            ' first.')
    ...
    X = _choose_representation(
        adata, neigh_params.get('use_rep', None), neigh_params.get(
            'n_pcs', None), silent=True)
    metric = adata.uns['neighbors']['params']['metric']
    if metric not in ['euclidean', 'manhattan', 'cosine', 'hamming']:
        raise ValueError('metric \'' + metric + '\' is currently not supported '
